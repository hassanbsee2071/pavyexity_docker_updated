    ...
    <div class="row">
        <table class="table table-striped table-hover">
            ...
            <tr>
                ...
                <td>{{ date('m/d/y', strtotime($user->created_at)) }} ({{ $user->created_at->diffForHumans() }})</td>
            </tr>

            <tr>
                <th>{{ __('views.admin.users.show.table_header_7') }}</th>
                <td>{{ date('m/d/y', strtotime($user->updated_at)) $user->updated_at }} ({{ $user->updated_at->diffForHumans() }})</td>
            </tr>

            <tr>
                <th>{{ __('views.admin.users.show.table_header_8') }}</th>
                <td>{{ date('m/d/y', strtotime($user->created_at)) }} ({{ $user->last_login->diffForHumans() }})</td>
            </tr>
            </tbody>
        </table>
    </div>
